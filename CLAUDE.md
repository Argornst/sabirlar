...
13. **Shared ghost button light theme hover bug** — `ui.css:1297` `.ui-button--ghost:hover` rengi `#1e3a8a` (mavi) tanımlı. Tüm projedeki ghost butonlarda light tema hover'da metin maviye dönüyor. Olması gereken slate (`#334155`) veya secondary text token'ı. Faz 3.5'te dispatch için scope'lu override yapıldı (`productions.css`'te `dispatch-view-tab--active`, `dispatch-week-nav-active`, `.dispatch-week-board__nav .ui-button--ghost:hover` için yüksek-specificity rule'lar), ama shared primitive temizliği gerekli — diğer modüllerdeki ghost butonlar hâlâ etkilenir.
